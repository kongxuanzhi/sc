//NO_STANDARD_RESOLVER //沒定义
//NO_FANCY_RESOLVER   //没定义
//HAVE_IPv6  //polipo.h 中根据系统的不同定义
//WIN32		 //windows
#include "polipo.h"
// https://www.irif.fr/~jch/software/polipo/polipo.html#The-domain-name-service
//errno.h头文件中定义
//EINVAL 参数不合法 22
//#define ENAMETOOLONG 36 /* File name too long */
//#define ENOMEM 12 /* Out of memory */

 //Its precise behaviour is controlled by the value of dnsUseGethostbyname. 
 //If dnsUseGethostbyname is false, Polipo never uses the system resolver. 
 //If it is reluctantly (the default), Polipo tries to speak DNS and falls back to the system resolver 
 //if a name server could not be contacted. 
 //If it is happily, Polipo tries to speak DNS, and falls back to the system resolver 
 //if the host couldn’t be found for any reason (this is not a good idea for shared proxies). 
 //Finally, if dnsUseGethostbyname is true, Polipo never tries to speak DNS itself 
 //and uses the system resolver straight away (this is not recommended).
...
//If the internal DNS support is used, 
//Polipo must be given a recursive name server to speak to. 
//By default, this information is taken from the ‘/etc/resolv.conf’ file at startup; 
//however, if you wish to use a different name server, 
//you may set the variable dnsNameServer to an IP address.
#ifndef NO_FANCY_RESOLVER
	AtomPtr dnsNameServer = NULL; //string存放字符串ip地址
    //When the reply to a DNS request is late to come, 
    //Polipo will retry multiple times using an exponentially increasing timeout. 
    //The maximum timeout used before Polipo gives up is defined by dnsMaxTimeout (default 60s); 
    //the total time before Polipo gives up on a DNS query will be roughly twice dnsMaxTimeout.
	int dnsMaxTimeout = 60;
#endif
//If the system resolver is used,
// the value dnsGethostbynameTtl specifies the time 
//during which a gethostbyname reply will be cached (default 5 minutes).
#ifndef NO_STANDARD_RESOLVER
	int dnsGethostbynameTtl = 1200;
#endif

//specifies the time during which negative DNS information 
//(information that a host doesn’t exist) will be cached; 
//Increasing this value reduces both latency and 
//network traffic but may cause a failed host not to be noticed when it comes back up.
int dnsNegativeTtl = 120; //defaults to 120s

//The variable dnsQueryIPv6 specifies whether to query for IPv4 or IPv6 addresses. 
//If dnsQueryIPv6 is false, only IPv4 addresses are queried. 
//If dnsQueryIPv6 is reluctantly, both types of addresses are queried, 
//but IPv4 addresses are preferred. 
//If dnsQueryIPv6 is happily (the default), IPv6 addresses are preferred. 
//Finally, if dnsQueryIPv6 is true, only IPv6 addresses are queried.
#ifdef HAVE_IPv6
	int dnsQueryIPv6 = 2;
#else
...
#endif
//staticref: do_gethostbyname | really_do_dns | dnsGethostbynameFallback
...
initDns()
{
	...
	//inet_aton是将一个字符串IP地址转换为一个32位的网络序列IP地址。
                 ...
                 void *data)
{
    ...
    if(n <= 0 || n > 131) { //处理错误情况
        if(n <= 0) { //case 1
            ...
            done = handler(-EINVAL, &request); //-EINVAL == -22
        } else { //case 2
            request.error_message = internAtom("name too long");
            do_log(L_ERROR, "DNS name too long.\n");
            done = handler(-ENAMETOOLONG, &request);
        }
        assert(done); //保证正确的done 才释放request.error_message
        releaseAtom(request.error_message);
        return 1;
    }

    if(origname[n - 1] == '.') //去掉name的最后一个点
        n--;

    name = internAtomLowerN(origname, n); //AtomPtr

    if(name == NULL) { //每一步都可能出错，尤其在创建元素，申请内存的时候
        request.error_message = internAtom("couldn't allocate name");
        do_log(L_ERROR, "Couldn't allocate DNS name.\n");
        done = handler(-ENOMEM, &request); //out of memory
        assert(done); //确保正确处理才释放
        releaseAtom(request.error_message);
        return 1;
    }
	//重新设置
    ...
    request.error_message = NULL;
    request.count = count;
    request.object = NULL; //设置object为null
    request.handler = handler;
    request.data = data;
	
	//int type, const void *key, int key_siz
    object = findObject(OBJECT_DNS, name->string, name->length);
	//优先处理错误的情况
	//判断object==NULL是很有必要的
	//ObjectPtr object, CacheControlPtr cache_control 重新验证 tbd
    if(object == NULL || objectMustRevalidate(object, NULL)) {
       //在分两种情况 object==NULL， object！=null Revalidate=false
		if(object) { //后一种情况要清空操作
            privatiseObject(object, 0); //私有化Object
            releaseObject(object); //释放Object
        }
		//这两种情况下，都要重新创建一个object
	...
	//这时候，object一定是找到了findObject
	// | 条件是两个二进制中的1合并， & 1和0抵消，判断相同位置是否有同一个1存在，
	//也就是有一个条件发生了 OBJECT_INITIAL 或者 OBJECT_INPROGRESS
    //如果是OBJECT_INITIAL情况(object hasn't got any headers yet)
	if((object->flags & (OBJECT_INITIAL | OBJECT_INPROGRESS)) == OBJECT_INITIAL) {
		//分情况调用really_do_gethostbyname
        if(dnsUseGethostbyname >= 3) // 1， 3， 0
			//组装好了name和object之后
            rc = really_do_gethostbyname(name, object);//这个才是really， 修改object
        else
            rc = really_do_dns(name, object);
        //失败
		if(rc < 0) {
			//object->flags既不等于OBJECT_INITIAL也不等于OBJECT_INPROGRESS
            assert(!(object->flags & (OBJECT_INITIAL | OBJECT_INPROGRESS)));
            goto fail;
        }
    }
	//really_do_gethostbyname执行完这个后object->flags ！= OBJECT_INITIAL
    if(dnsUseGethostbyname >= 3) {
		//object->flags 不要等于OBJECT_INITIAL了
        assert(!(object->flags & OBJECT_INITIAL));
	}
#ifndef NO_FANCY_RESOLVER   
	//object->flags == OBJECT_INITIAL 仍等于
    if(object->flags & OBJECT_INITIAL) {
		//因某种条件而等待
        ConditionHandlerPtr chandler;
        assert(object->flags & OBJECT_INPROGRESS);
		//object->flags 又等于 OBJECT_INPROGRESS
...
#endif
	//object->flags 不要等于OBJECT_INITIAL
	//object has got any headers. //存放着hosts
    if(object->headers && object->headers->length > 0) {
        if(object->headers->string[0] == DNS_A) //0
            assert(((object->headers->length - 1) % 
                    sizeof(HostAddressRec)) == 0);
        else
            assert(object->headers->string[0] == DNS_CNAME); //1
        request.addr = retainAtom(object->headers); //retainAtom 保留 //主要目的
    } else if(object->message) { //报错
        request.error_message = retainAtom(object->message);
    }
	//object的任务结束了， 主要给request设置参数
    releaseObject(object);
	//handler处理request
	//request设置了发、name ，addr，error_message, handler, object, data
    if(request.addr && request.addr->length > 0)
        done = handler(1, &request); //正常情况下处理请求
    else
...
#ifdef HAVE_IPv6
//staticref: really_do_gethostbyname | really_do_dns 
AtomPtr
...
//staticref: really_do_gethostbyname
int
...
really_do_gethostbyname(AtomPtr name, ObjectPtr object)
{
	...
	/*struct hostent
	{
		char    *h_name; //www.a.shifen.com
		char    **h_aliases; //www.baidu.com
		int     h_addrtype; //
		int     h_length;
		char    **h_addr_list; 121.14.89.11
		#define h_addr h_addr_list[0]
	};*/
	//主要作用： 将通过gethostbyname获得的host的h_addr_list数组字段，转化为AtomPtr类型
	//存储到object的headers参数中，修改object的flag， age(设置header日期)和expires(过期时间)
    struct hostent *host;
    char *s;
    AtomPtr a;
    int i, j;
    int error;
	//gethostbyname: http://www.cnblogs.com/kex1n/p/5524644.html
	/*  ./ a.out www.baidu.com
		official hostname : www.a.shifen.com
		alias : www.baidu.com
		address : 121.14.89.11
		first address : 121.14.88.11*/
    host = gethostbyname(name->string); //#include <netdb.h>

	// h_errno
    if(host == NULL) {
        switch(h_errno) { //#include <netdb.h>
    ...
    s = malloc(1 + i * sizeof(HostAddressRec)); //返回char*可以逐字节按数组操作
    if(s == NULL) {
        a = NULL;
    } else {
        ...
        s[0] = DNS_A; //第0个字节
        for(j = 0; j < i; j++) {
            s[j * sizeof(HostAddressRec) + 1] = 4; //第一个字节af
            memcpy(&s[j * sizeof(HostAddressRec) + 2], host->h_addr_list[j], 
                   sizeof(struct in_addr)); //从第2个字节开始往后，struct in_addr
        }
        a = internAtomN(s, i * sizeof(HostAddressRec) + 1); //把s存储在a中
        free(s);
    }
    if(!a) {
        object->flags &= ~OBJECT_INPROGRESS;
    ...
    object->headers = a;
    object->age = current_time.tv_sec; //生成日期
    object->expires = current_time.tv_sec + dnsGethostbynameTtl; //1200ms后过期
    object->flags &= ~(OBJECT_INITIAL | OBJECT_INPROGRESS); //消除 
	//~(OBJECT_INITIAL | OBJECT_INPROGRESS); /除此之外的其他事件， 求&，就是清空flags，保留其他事件标记
    notifyObject(object);
    return 0;
...
//staticref: dnsTimeoutHandler | really_do_dns | dnsReplyHandler
static void
...
//staticref: dnsReplyHandler | really_do_dns
static int
establishDnsSocket()
{
    int rc;
...
//staticref: dnsTimeoutHandler | really_do_dns
static int
...
//staticref: do_gethostbyname
//name: www.baidu.com | localhost | localhost.
//object: 初始化 name
//operate: object.headers, age, expires, flag
//return: object.header 127.0.0.1 ipv4 DNS_A
static int really_do_dns(AtomPtr name, ObjectPtr object)
{
    int rc;
    DnsQueryPtr query;
    ...
    int id;
    AtomPtr a = NULL;
	//可能是这里如果加入其他的代码，a的值可能就改变了
    if(a == NULL) { //a 一定为null，为什么还要判断
        if(name == atomLocalhost || name == atomLocalhostDot) { //如果name=localhost
            char s[1 + sizeof(HostAddressRec)];
            memset(s, 0, sizeof(s));
            s[0] = DNS_A;
            s[1] = 4;
