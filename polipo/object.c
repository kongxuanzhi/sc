...
static ObjectPtr *objectHashTable; //主要存储object的hash表
int objectHashTableSize = 0;
int log2ObjectHashTableSize;
int maxExpiresAge = (30 * 24 + 1) * 3600; //20天加一小时
int maxAge = (14 * 24 + 1) * 3600; //14天加一小时
    ...
    if(objectHashTableSize <= objectHighMark / 2 ||
       objectHashTableSize > objectHighMark * 1024) {
        ...
        if(objectHashTableSize != 0)
			q = 0;
        objectHashTableSize = objectHighMark * 16; //默认初始那么大的内存
    }
